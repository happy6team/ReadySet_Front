...
const threadId = '1'
const projectName = '스마트팜 스토어'
const projectExplain = '스마트팜을 만든다'
...
const sendMessage = async () => {
  ...
  // 3) 요청 준비
  const url = '/api/chat'  // 슬래시로 시작하는 절대경로 권장
  const requestBody = {
    input_query: userInput.value,
    thread_id: threadId,
  ...
  try {
    ...
    const aiText = response.data.messages?.[0]?.content ?? '응답이 없습니다.'
    ...
    nextTick(() => {
      ...
      if (chatContainer) {
        chatContainer.scrollTop = (chatContainer as any).scrollHeight
      }
    })
  }
}
